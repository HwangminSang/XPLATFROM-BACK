...
const getListData = (divisionCodeNo) => {
    ...
    xhr.open('GET',
        '/base/codeList?method=findDetailCodeList&divisionCodeNo='+divisionCodeNo,
        true);
...
const getStandardUnitPrice = (itemCode, unit) => {
    ...
    // BOX
    xhr.open('GET',
        `/logisticsInfo/getStandardUnitPrice?method=getStandardUnitPriceBox&itemCode=${itemCode}`,
        true);
    xhr.setRequestHeader('Accept', 'application/json');
    xhr.send();
    xhr.onreadystatechange = () => {
...
const getMrpList = (mpsRowNode) => {
	//mrpGridOptions.api.refreshHeader();
    ...
    xhr.open('POST', '/production/openMrp' +
        "?method=openMrp"
...
const registerMrp = (mrpDate) => {  // mrp등록 (소요량 전개 일자)
	...
	console.log(mpsNoList);
    xhr.open('POST', '/production/registerMrp' +
        "?method=registerMrp"
...
const getMrpGatheringModal = (mrpNoList) => {  // 소요량취합결과 데이터를 받아와 그리드에 세팅 
    ...
    xhr.open('GET', '/production/getMrpGatheringList' +
...
const registerMrpGathering = (mrpGatheringDate, mrpNoList, mrpNoAndItemCodeList) => {
    ...
    xhr.open('POST', '/production/registerMrpGathering' +
...
const getOrderModal = (mrpGatheringList) => {
    ...
    xhr.open('POST', '/material/showOrderDialog' +
...
const getWarehousingModal = () => {
    let xhr = new XMLHttpRequest();
    xhr.open('GET', '/material/searchOrderInfoListOnDelivery' +
...
const getWorkOrderSimulationModal = (mrpGatheringNo,mrpNo) => {
    let xhr = new XMLHttpRequest();
    xhr.open('GET', '/production/showWorkOrderDialog'
...
const getWorkSiteModal = (workSiteCourse, workOrderNo, itemClassIfication) => { // 검사항목,작업지시번호,품목분류
    ...
    let xhr = new XMLHttpRequest();
    xhr.open('POST', '/production/showWorkSiteSituation'
